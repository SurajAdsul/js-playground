...
function App() {
  ...
  const executeCode = async (codeToExecute) => {
    ...
    try {
      ...
      try {
        ...
        for (const line of lines) {
          if (line.includes('console.log') && !line.includes('console.log(')) {
            throw new Error(
              'Invalid console.log usage. Did you forget parentheses? Use console.log("message") instead of console.log'
            )
          }
        }
      ...
      const hasComplexCode =
        codeToExecute.includes('function') ||
        codeToExecute.includes('=>') ||
  ...
  const togglePackageManager = () => {
    ...
    setShowSettings(false) // Close settings menu after opening package manager
  }
    ...
    >
      <div className="header">
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button
            className={`button toggle ${preferences.autoExecute ? 'active' : ''}`}
            onClick={() => {
              const newAutoExecute = !preferences.autoExecute
              setPreferences((prev) => ({
                ...prev,
                autoExecute: newAutoExecute
              }))
              window.electron.preferences.save({
                ...preferences,
                autoExecute: newAutoExecute
              })
            }}
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="button secondary" onClick={togglePackageManager}>
            Package Manager
          </button>
          <button className="button secondary" onClick={handleOpenPreferences}>
            Preferences
          </button>
        </div>
      </div>
      ...
      >
        <div
          ...
          style={{
            height: '100%',
            backgroundColor: currentTheme === 'vs-light' ? '#ffffff' : '#282a36'
          }}
        >
          <CodeMirror
            value={code}
      ...
      {showPackageManager && (
        <ErrorBoundary>
          <PackageManager onClose={togglePackageManager} currentTheme={currentTheme} />
        </ErrorBoundary>
      )}
